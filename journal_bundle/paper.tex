%% ============================================================================
%% paper.tex -- JMP submission bundle (REVTeX 4.2, AIP)
%% "The WKB Geometry of Newton Polygons: A Functorial Classification Theory"
%% Version: v2.0
%% Companion dataset: doi:10.5281/zenodo.20387847
%% ============================================================================

\documentclass[aip,jmp,preprint,amsmath,amssymb,nofootinbib,floatfix]{revtex4-2}

%% --- Additional math packages (REVTeX preloads amsmath/amssymb) ----------
\usepackage{amsthm}
...
\author{Papanokechi%
  ...
  Companion dataset:
  \href{https://doi.org/10.5281/zenodo.20387847}{10.5281/zenodo.20387847}.}%
}
...
polynomial $\chi \in \mathbb{C}[c]$. Our central algebraic identity is $\chi(c) = \chi_w(c^q)$, where $\chi_w \in \mathbb{C}[w]$ is the classical edge-indicial polynomial
...
are bundled in the dataset as the file \texttt{proofs.md}, in long-form
expository style with explicit cross-references to Phase-III/IV evidence.
The integration map (\texttt{integration\_map.md}) records, for each
theorem, the dataset artifact that supplies its empirical evidence. The
counterexample catalogue (\texttt{counterexamples.json}) records the
strata on which naive forms of the theorems fail; each theorem statement
above includes a remark guarding against the relevant counterexamples.

%% ============================================================================
%% Acknowledgements (incl. compact AI-disclosure paragraph)
%% ============================================================================
\section*{Acknowledgements}\label{sec:ack}

The author thanks the contributors to \texttt{sympy}, \texttt{numpy},
and \texttt{matplotlib}, on which the fleet pipelines that produced
the computational atlas rest.

\paragraph{Computational environment.}
All numerical and symbolic work in this paper was carried out in
\texttt{Python 3.10+} using \texttt{sympy}, \texttt{numpy},
\texttt{fractions}, and \texttt{matplotlib}. No randomised algorithms
were used and no random seeds were set; every JSON artifact in the
companion dataset \cite{wkb_dataset_v1} is reproduced bytewise on
re-execution. The manuscript was typeset with MiKTeX/\TeX{} Live; the
bibliography was processed with \texttt{bibtex} against
...
model). The assistant was used for drafting prose, organising
exploratory invariant tables produced by the fleet pipelines, unifying
notation across revisions, and assembling the submission bundles.
\emph{All mathematics, theorem statements, proofs, and classification
claims in this paper were authored, verified, and finalised by the
human researcher;} the AI tool did not generate any load-bearing
mathematical argument or numerical result. No proprietary or
closed-source models were used beyond the publicly-available AI
assistant named above; no model was fine-tuned on the project's data,
and no AI-mediated literature summaries were used in place of primary
sources for citations. The author retains sole accountability for all
claims, methodology choices, and the published result.

%% ============================================================================
%% Author Declarations
%% ============================================================================
\section*{Author Declarations}\label{sec:declarations}

\paragraph{Conflict of Interest.}
The author has no conflicts of interest to disclose.

\paragraph{Author Contributions.}
\textbf{Papanokechi:} Conceptualization (lead); Data curation (lead);
Formal analysis (lead); Investigation (lead); Methodology (lead);
Software (lead); Validation (lead); Visualization (lead);
Writing -- original draft (lead); Writing -- review \& editing (lead).
This is a sole-authored manuscript; all roles were carried out by the
sole author.
...
available in the Zenodo repository at
\href{https://doi.org/10.5281/zenodo.20387847}{doi:10.5281/zenodo.20387847}
(version 2.0), and are mirrored in the GitHub repository at
\url{https://github.com/papanokechi/wkb-newton-polygons} (tagged
release \texttt{v2.0}). The deposit contains the source pipelines
(\texttt{fleets/}), JSON datasets (\texttt{data/}), phase reports
(\texttt{reports/}), figures, and the manuscript sources required to
reproduce every result reported in this paper. See
